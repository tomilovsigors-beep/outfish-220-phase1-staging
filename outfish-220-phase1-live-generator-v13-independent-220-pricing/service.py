...
def refresh_once():
    with lock:
        ...
        try:
            ...
            # V13: 220 price is independent from Shopify price.
            pricing_report = apply_independent_220_pricing(
                tmp / 'stock-price-candidate.xml',
                PRODUCTION_FEED_URL,
                Path(__file__).resolve().parent / '220_price_overrides.csv',
            )
            (tmp / 'marketplace-pricing-report.json').write_text(
                json.dumps(pricing_report, ensure_ascii=False, indent=2),
                encoding='utf-8',
            )
            ...
            # Build a full production-sized feed: replace only the 672 approved
            # Phase 1 rows, preserve every other production row unchanged.
            merge_report = merge_candidate_into_production(
                tmp / 'stock-price-candidate.xml',
                PRODUCTION_FEED_URL,
                tmp / '220-stock-merged.xml',
            )
            (tmp / 'production-merge-report.json').write_text(
            ...
            cutover_gate = {
                ...
                'cutover_allowed': bool(gate.get('publish_allowed')) and bool(prod_diff.get('pass')) and bool(merge_report.get('pass')),
                'generator_publish_allowed': bool(gate.get('publish_allowed')),
                'production_compare_pass': bool(prod_diff.get('pass')),
                'production_merge_pass': bool(merge_report.get('pass')),
                'independent_220_pricing': True,
                'shopify_price_used_for_220': bool(pricing_report.get('shopify_price_used')),
                'explicit_price_overrides': pricing_report.get('explicit_price_overrides'),
                'merged_rows': merge_report.get('merged_rows'),
                'untouched_rows': merge_report.get('untouched_rows'),
                'expected_export_rows': EXPECTED_EXPORT_ROWS,
                'candidate_rows': prod_diff.get('candidate_rows'),
                'production_rows': prod_diff.get('production_rows'),
                'blocking_checks': ([x for x in prod_diff.get('checks', []) if not x.get('pass')] + [x for x in merge_report.get('checks', []) if not x.get('pass')]),
                'safety': {
                    'publishes_to_220': False,
                    'replaces_production_feed': False,
                    'full_feed_merge_ready': bool(merge_report.get('pass')),
                    'independent_220_pricing': True,
                    'shopify_price_used_for_220': False,
                    'modifies_only_candidate_skus': bool(merge_report.get('pass')),
...
def status_payload():
    ...
    return {
        'service': 'outfish-220-phase1-staging-v13-independent-220-pricing',
        'mode': MODE,
...
class Handler(BaseHTTPRequestHandler):
    server_version = 'Outfish220Staging/13'
    ...
    def do_GET(self):
        ...
        if self.path == '/production-merge-report.json':
            report = load_json(CURRENT / 'production-merge-report.json', {}) or {}
            return self._send_json(200 if report else 404, report if report else {'error':'no merge report yet'})
        if self.path == '/marketplace-pricing-report.json':
            report = load_json(CURRENT / 'marketplace-pricing-report.json', {}) or {}
            return self._send_json(200 if report else 404, report if report else {'error':'no marketplace pricing report yet'})
        if self.path == '/ready':
            payload = status_payload()
            return self._send_json(200 if payload.get('cutover_allowed') else 503, payload)
        if self.path == '/220-stock-merged.xml':
            gate = current_gate()
            path = CURRENT / '220-stock-merged.xml'
            if not path.exists():
                return self._send_json(404, {'error':'merged feed not generated yet'})
            if not gate.get('cutover_allowed'):
                return self._send_json(503, {'error':'merged feed blocked by cutover gate', 'gate':gate})
