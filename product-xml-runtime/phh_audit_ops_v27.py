...
def run():
    ...
    for path,methods in (spec.get('paths') or {}).items():
        ...
        if not any(k in hay for k in ('product-modification','product modification','barcodes','seller offer','offers')):
            continue
    ...
    print('PHH_AUDIT_OPS_V27 '+json.dumps({'status':'PASS','operations':out},ensure_ascii=False,sort_keys=True),flush=True)
