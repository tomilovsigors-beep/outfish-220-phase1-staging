...
from pmp_api_probe import _api_get, _api_login
...
def export_all(version="v3", limit=100):
    login = _api_login(version)
    if login is None:
    ...
    while total_count is None or offset < total_count:
        response = _api_get(f"/{version}/categories?limit={limit}&offset={offset}", token, timeout=45)
        if not response.ok:
            raise RuntimeError(f"categories page failed at offset={offset}: HTTP {response.status_code}")
        data = response.json()
        if not isinstance(data, dict):
            raise RuntimeError("categories response is not an object")
        page = data.get("category_list") or []
        ...
        if total_count is None:
            ...
            if total_count <= 0:
                raise RuntimeError("invalid total_count in categories response")
    ...
    for c in categories:
        ...
        for a in attrs:
            if not isinstance(a, dict):
                continue
            extra_attribute_keys.update(set(a.keys()) - EXPECTED_ATTRIBUTE_KEYS)
            fid = a.get("field_id")
            if fid is not None:
            ...
            attribute_rows.append({
                ...
                "title_lt": a.get("title_lt") or "",
                "title_lv": a.get("title_lv") or "",
                "title_ee": a.get("title_ee") or "",
                "title_fi": a.get("title_fi") or "",
                "title_ru": a.get("title_ru") or "",
                "extra_json": json.dumps({k: a.get(k) for k in sorted(set(a.keys()) - EXPECTED_ATTRIBUTE_KEYS)}, ensure_ascii=False, sort_keys=True),
            })
    ...
    for c in categories:
        row = {
            ...
            "title_lv": c.get("title_lv") or "",
            "title_lt": c.get("title_lt") or "",
            "title_ee": c.get("title_ee") or "",
            "title_fi": c.get("title_fi") or "",
            "title_ru": c.get("title_ru") or "",
            "title_pl": c.get("title_pl") or "",
        }
    ...
    summary = {
        ...
        "leaf_addable_categories": len(leaf_rows),
        "non_addable_categories": len(categories) - len(leaf_rows),
        ...
        "unique_field_ids": len(unique_field_ids),
        "unique_required_field_ids": len(required_field_ids),
        "max_attribute_count_per_category": max_attribute_count,
    ...
    category_fields = [
        "category_id", "parent_id", "category_path", "allow_add_products",
        "attribute_count", "required_attribute_count", "title_en", "title_lv",
        "title_lt", "title_ee", "title_fi", "title_ru", "title_pl",
    ]
    attribute_fields = [
        "category_id", "category_path", "allow_add_products", "field_id", "required",
        "title_lt", "title_lv", "title_ee", "title_fi", "title_ru", "extra_json",
    ]
    artifacts = {
        "pmp-categories-full.json": json.dumps({"summary": summary, "category_list": categories}, ensure_ascii=False, sort_keys=True, indent=2).encode("utf-8"),
        ...
        "pmp-category-export-summary.json": json.dumps(summary, ensure_ascii=False, sort_keys=True, indent=2).encode("utf-8"),
    }
