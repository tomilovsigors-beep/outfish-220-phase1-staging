...
def run():
    ...
    for path,methods in (spec.get('paths') or {}).items():
        ...
        if any(k in hay for k in ('category feature','feature value','features values','category fields','field values','categories/')):
    ...
    print('PHH_FEATURE_VALUE_OPS_V23 '+json.dumps({'status':'PASS','operations':out},ensure_ascii=False,sort_keys=True),flush=True)
