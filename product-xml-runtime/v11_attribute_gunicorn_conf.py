...
def _register_routes(server):
    try:
        ...
        routes={
          '/v11/v11-required-field-source-detail.csv':('v11-required-field-source-detail.csv','text/csv'),
          '/v11/v11-product-attribute-readiness.csv':('v11-product-attribute-readiness.csv','text/csv'),
          '/v11/v11-field-gap-matrix.csv':('v11-field-gap-matrix.csv','text/csv'),
          '/v11/v11-category-readiness.csv':('v11-category-readiness.csv','text/csv'),
          '/v11/v11-attribute-source-summary.json':('v11-attribute-source-summary.json','application/json'),
        }
        for idx,(path,(name,mime)) in enumerate(routes.items()):
            endpoint=f'v11_attr_artifact_{idx}'
        ...
        server.log.info('V11_ATTRIBUTE_ARTIFACT_ROUTES_READY %s',json.dumps(sorted(routes)))
    except Exception as exc:
        server.log.warning('V11_ATTRIBUTE_ARTIFACT_ROUTES_FAILED %s %s',type(exc).__name__,str(exc)[:3000])
...
def when_ready(server):
    ...
    if os.getenv('RUN_V11_ATTRIBUTE_SOURCE_AUDIT','').strip()=='1':
        server.log.info('V11_ATTRIBUTE_SOURCE_AUDIT_HOOK_START')
        def _audit():
            try:
                from app import _master_rows, _shopify_products
                from current_product_attribute_source_audit_v11 import run_audit
                master=_master_rows(); shopify=_shopify_products(master)
                summary,_=run_audit(master,shopify,os.getenv('DATABASE_URL'))
                server.log.info('V11_ATTRIBUTE_SOURCE_AUDIT_COMPLETE %s',json.dumps(summary,sort_keys=True))
            except Exception as exc:
