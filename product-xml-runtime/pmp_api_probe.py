...
def _docs_get(path, timeout=25):
    return requests.get(
        ...
        auth=_docs_creds(),
        timeout=timeout,
        headers={"User-Agent": "outfish-phh-readonly-discovery/7.0"},
    )
...
def _api_get(path, token, timeout=25):
    return requests.get(
        ...
        timeout=timeout,
        headers={
            "User-Agent": "outfish-phh-readonly-discovery/7.0",
...
def discover():
    ...
    result = {
        "docs": {
            "status": docs.status_code,
            "bytes": len(docs.content),
            "content_type": docs.headers.get("content-type"),
        }
    }
    ...
    semantic_schemas = {
        ...
        if any(w in k.lower() for w in schema_words)
    }

    # API credentials are intentionally separate from HTTP Basic docs credentials.
    # Authentication POST only obtains a token; it does not mutate marketplace data.
    ...
    result["openapi_summary"] = {
        "source": "embedded_openapi",
        "embedded_spec_parsed": True,
        "openapi": spec.get("openapi"),
        "title": (spec.get("info") or {}).get("title"),
        "version": (spec.get("info") or {}).get("version"),
        "path_count": len(paths),
        "login_operation": {
            "summary": login_op.get("summary"),
            "description": login_op.get("description"),
            "requestBody": login_op.get("requestBody"),
            "responses": login_op.get("responses"),
        },
        "category_operation": {
            "summary": category_op.get("summary"),
            "parameters": category_op.get("parameters"),
            "responses": category_op.get("responses"),
        },
        "semantic_paths": _semantic_paths(paths),
        "semantic_schemas": semantic_schemas,
        "api_login_probe": login_info,
        "categories_probe": cat_info,
