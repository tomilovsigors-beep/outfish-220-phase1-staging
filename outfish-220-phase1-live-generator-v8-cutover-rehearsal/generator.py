...
def route_stock(store_raw: int, lowa_raw: int, fjord_raw: int) -> dict:
    ...
    return {
        ...
        "selected_stock": 0,
        "marketplace_stock": 0,
        # XML requires a value; keep 24 for out-of-stock rows, while stock_source remains NONE.
        "collectionhours": 24,
        "stock_source": "NONE",
    }
...
def diff_rows(rows: List[dict]) -> List[dict]:
    ...
    for r in rows:
        out.append({
            ...
            "generated_220_price": r.get("required_220_selling_price"),
...
def self_test() -> int:
    ...
    assert route_stock(0, 0, 0)["marketplace_stock"] == 0
    pilot_test = [{
