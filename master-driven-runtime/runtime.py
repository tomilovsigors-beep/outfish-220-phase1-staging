...
_state_lock=threading.RLock(); _state={'service_status':'starting','stale':False,'last_attempt':None,'last_successful_refresh':None,'error':None,'validated_dir':None,'audit':{}}
...
def fetch_sia():
    return fetch_sheet_csv(os.getenv('SIA_SHEET_ID','1qWs8saSIlq1TLhcg-BLZ1dWHRojDQWRX2BlEpZTkDjE'),os.getenv('SIA_GID','0'),'SIA_CSV_URL')
...
def fetch_shopify(mapped):
    ...
    if len(gids)!=970 or len(set(gids))!=970: raise RuntimeError('locked Shopify GID count/uniqueness gate failed')
    out={}; snap=[]; unknown=set()
    for i in range(0,len(gids),50):
        ...
        for gid,node in zip(batch,nodes):
            ...
            levels={STORE:None,LOWA:None,FJORD:None}; inv=(node.get('inventoryItem') or {}).get('inventoryLevels') or {}
            ...
            for lv in inv.get('nodes') or []:
                loc=lv.get('location') or {}; lid=loc.get('id'); name=loc.get('name'); qv=next((x for x in (lv.get('quantities') or []) if x.get('name')=='on_hand'),None); raw=None if qv is None else qv.get('quantity')
                if lid in levels: levels[lid]=raw
                elif raw not in (None,0): unknown.add(f'{lid}|{name}|{raw}')
                snap.append({'variant_gid':gid,'inventory_item_gid':(node.get('inventoryItem') or {}).get('id',''),'location_gid':lid or '','location_name':name or '','raw_on_hand':'' if raw is None else raw,'source_fetch_status':'OK','fetched_at':now()})
            out[gid]=levels
...
def refresh():
    started=now(); run=SNAP/started.replace(':','').replace('+','_'); run.mkdir(parents=True,exist_ok=True)
    with _state_lock:_state.update(last_attempt=started,service_status='refreshing',error=None)
    try:
        ...
        if len(external)!=701: raise RuntimeError(f'external source assignment {len(external)}/701')
        sia_raw,srows=fetch_sia(); sia={norm(r.get('sku')):r for r in srows if norm(r.get('sku'))}
        missing_external=[s for s in external if s not in sia]
        if missing_external: raise RuntimeError(f'SIA external incomplete {len(missing_external)}')
        shop,snap,unknown=fetch_shopify(mapped)
        rows=[]
        for sku in skus:
            ...
            errors+=price_errors(m)
            selected=''; stock=''; hours=''; rs=rl=rf=re=''; zero_hours=norm(sr.get('collectionhours'))
            if sku in mapped:
                gid=mapped[sku]; lv=shop.get(gid)
                if lv is None: errors.append('SHOPIFY_SNAPSHOT_ROW_MISSING')
                else:
                    rs,rl,rf=lv.get(STORE),lv.get(LOWA),lv.get(FJORD); selected,stock,hours,ee=route_shop(lv,zero_hours); errors+=ee
        ...
        sf=list(snap[0]) if snap else ['variant_gid']; shopb=csv_bytes(snap,sf)
        diffs=[]
        for r in rows:
            sr=sia.get(r['220_sku'],{}); bs=norm(sr.get('stock')); bh=norm(sr.get('collectionhours')); bp=norm(sr.get('(1) price before discount')); ap=norm(sr.get('(2) price after discount'))
            pd=(norm(r['220_price_before_discount'])!=bp or norm(r['220_price_after_discount'])!=ap)
        ...
        diffb=csv_bytes(diffs,list(diffs[0])); dh=canon_hash(rows,fields)
        for name,b in [('master.csv',master_raw),('sia.csv',sia_raw),('shopify_inventory_snapshot.csv',shopb),('runtime-dataset.csv',dataset),('blockers.csv',csv_bytes(blocked,fields)),('220-stock-master-driven.xml',xml),('production-equivalent-comparison.csv',diffb)]: (run/name).write_bytes(b)
        audit={'service_status':'ok','stale':False,'refresh_id':run.name,'source_timestamps':{'refresh_started_at':started,'refresh_completed_at':now()},'source_snapshot_ids':{'master_sha256':sha(master_raw),'sia_sha256':sha(sia_raw),'shopify_snapshot_sha256':sha(shopb)},'identity_coverage':'1671/1671','source_coverage':'1671/1671','master_identities':1671,'shopify_mapped_expected':970,'shopify_fetched':len(shop),'sia_external_expected':701,'sia_external_fetched':701-len(missing_external),'missing_shopify_gid_count':970-len(shop),'duplicate_shopify_gid_count':970-len(set(mapped.values())),'unknown_shopify_locations':unknown,'location_conflicts':sum('BLOCKED_LOCATION_CONFLICT_LOWA_FJORD' in r['blocker_reason'] for r in rows),'invalid_external_stock':sum('EXTERNAL_STOCK_VALUE_MISSING_OR_INVALID' in r['blocker_reason'] for r in rows),'zero_stock_hours_blockers':sum('BLOCKED_ZERO_STOCK_COLLECTIONHOURS_UNDEFINED' in r['blocker_reason'] for r in rows),'lifecycle_blockers':sum('LIFECYCLE_' in r['blocker_reason'] for r in rows),'price_blockers':sum('PRICE_' in r['blocker_reason'] for r in rows),'runtime_ok':len(ok),'blocked':len(blocked),'generated_xml_rows':len(ok),'dataset_sha256':dh,'shopify_price_used':False,'old_production_xml_dependency':False,'silent_fallback':False,'stock_diffs':sum(d['stock_diff']=='true' for d in diffs),'hours_diffs':sum(d['hours_diff']=='true' for d in diffs),'price_diffs':sum(d['price_diff']=='true' for d in diffs),'explicit_excluded_skus':sorted(EXPLICIT_BLOCKERS)}
        (run/'audit.json').write_text(json.dumps(audit,indent=2),encoding='utf-8'); (SNAP/'CURRENT').write_text(str(run),encoding='utf-8')
