    ...
    for src in pilot_rows:
        ...
        rec = {
            **base,
            ...
            "generated_price": required_price,
...
def stock_price_xml(rows: List[dict]) -> str:
    ...
    for r in rows:
        ...
        if price is None or stock is None or hours is None:
            continue
        price_s = money(r.get("generated_price"))
        if not price_s:
            continue
        parts += [
            "  <product>",
            ...
            f"    <price-before-discount>{price_s}</price-before-discount>",
            f"    <price-after-discount>{price_s}</price-after-discount>",
        ]
        parts += [
...
def diff_rows(rows: List[dict]) -> List[dict]:
    ...
    for r in rows:
        out.append({
            ...
            "generated_price": r.get("generated_price"),
            "price_raise_required": r.get("price_raise_required"),
